...
// 稀疏矩阵十字链的方法声明:
	...
	~CrossList();				// 析构函数
	void Clear();					// 清空稀疏矩阵
	inline int GetRows() const { return rows; };			// 返回稀疏矩阵行数
    inline int GetCols() const { return cols; };			// 返回稀疏矩阵列数
    inline int GetNum() const { return num; };			// 返回稀疏矩阵非零元个数
	Status SetElem(int r, int c, const ElemType &v);	// 设置指定位置的元素值
	Status GetElem(int r, int c, ElemType &v) const;			// 取指定位置的元素值
	...
	void Show(ostream& out = cout) const;							//以矩阵形式输出
	void Input(istream& in = cin);							//输入到矩阵
...
template<class ElemType> istream& operator >>(istream& in, CrossList<ElemType>& list);	//输入流重载
...
template<class ElemType>
void CrossList<ElemType>::Show(ostream &out) const
{
	ElemType v;
	for (int i = 0; i < rows; i++) 
	{
...
template<class ElemType>
void CrossList<ElemType>::Input(istream& in)							//输入到矩阵
{
	ElemType v;
	for (int i = 0; i < rows; i++) 
	{
...
template <class ElemType> 
void CrossList<ElemType>::Save(const char* filename) const		// 将链表所有结点的数据写入指定文件
{
	ofstream outfile;
...
template <class ElemType> 
void CrossList<ElemType>::Load(const char* filename)		// 从指定文件中读取数据，构造链表
{
...
template<class ElemType> 
ostream& operator <<(ostream& out, const CrossList<ElemType>& list)
{
	list.Show(out);
	return out;
}
...
template<class ElemType> 
istream& operator >>(istream& in, CrossList<ElemType>& list)	//输入流重载
{
	list.Input(in);
	return in;
}

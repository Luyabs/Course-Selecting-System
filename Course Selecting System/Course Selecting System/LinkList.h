...
template <typename T>
void LinkList<T>::GetList(istream& in,int n)			// 从输入设备in获取数据，形成链表。
{
	T x;
	for (int i = 0; i < n; i++)
	{
...
template <typename T>
void LinkList<T>::Save(const char* filename)		// 将链表所有结点的数据写入指定文件
{
...
template <typename T>
void LinkList<T>::Load(const char* filename)		// 从指定文件中读取数据，构造链表
{
